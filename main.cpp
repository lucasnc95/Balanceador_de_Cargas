...
int main(int argc, char *argv[])
{
	...
	for (int count = 0; count < todosDispositivos; count++)
	{
		if (count >= meusDispositivosOffset && count < meusDispositivosOffset + meusDispositivosLength)
		{
			...
			tempoLocalWriteByte = sizeCarga / (tempoFim - tempoInicio) * 2;
	...
	MPI_Allreduce(&tempoLocalWriteByte, &tempoGlobalWriteByte, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
	...
	for (int simulacao = 0; simulacao < SIMULACOES; simulacao++)
	{
		...
		if (balanceamento && ((simulacao == 0) || (simulacao == 1) || (simulacao % INTERVALO_BALANCEAMENTO == 0)))
		{
			...
			MPI_Allreduce(tempos, temposRoot, todosDispositivos, MPI_LONG, MPI_SUM, MPI_COMM_WORLD);
			...
			MPI_Allreduce(&tempoLocalLatencia, &tempoGlobalLatencia, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
			MPI_Allreduce(&tempoLocalBanda, &tempoGlobalBanda, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
